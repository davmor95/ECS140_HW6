...
% Base Case; Return the input MemList
fit1st([], MemList, NewMemList) :-
  NewMemList = MemList.

% If you see a list input, run fit1stRequest
fit1st([R|RList], MemList, NewMemList) :-
  R = [_,_],
  fit1stRequest(R, MemList, Return),
  fit1st(RList, Return, NewMemList).

% If you do not see a list input, run fitRelease
fit1st([R|RList], MemList, NewMemList) :-
  R \= [_,_],
  fitRelease(R, MemList, Return),
  fit1st(RList, Return, NewMemList).

% Base Case; Return the input MemList 
fitany([], MemList, NewMemList) :-
  NewMemList = MemList.

% If you see a list input, run fitanyRequest
fitany([R|RList], MemList, NewMemList) :-
  R = [_,_],
  fitanyRequest(R, MemList, Return),
  fitany(RList, Return, NewMemList).
...
% Test if RRList fails; if so, test all permutations on fit1st
...
% Test if RRList fails; if so, test all permutations on fitany 
