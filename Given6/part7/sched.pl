...
% Part 5 shells for built-in functions so they dont fail upon empty list input
myLast(List, Return) :-
  List == [],
  Return = [];
  last(List, Return).
...
fit1st([],MemList, NewMemList):-
 ...
 fit1st([Element|Remain], MemList, NewMemList) :-
 	Element = [_,_],
 	fit1stRequest(Element, MemList, Temp),
	fit1st(Remain, Temp, NewMemList).


fit1st([Element|Remain], MemList, NewMemList) :-
	Element \= [_,_],
 	fitRelease(Element, MemList, Temp),
 	fit1st(Remain, Temp, NewMemList).
...
fitany([],MemList, NewMemList):-
 ...
 fitany([Element|Remain], MemList, NewMemList) :-
 	Element = [_,_],
 	fitanyRequest(Element, MemList, Temp),
	fitany(Remain, Temp, NewMemList).
